...
**Radii past the apothem are dropped, in complementary pairs.** An arc centred on a vertex is an arc of the circle about that vertex, and the six triangles meeting there form a hexagon whose inscribed circle has radius `H` — so `rad ≤ H` is exactly the condition for the arc to stay inside its own triangle, and one comparison replaces any density threshold. Past it the clip takes the arc's middle out against the opposite edge and leaves a stub either side, which reads as breakage rather than as an arc. This bites from `k = 4` (one radius), and from `k = 12` two.

Dropping the outer `m` **requires** dropping the inner `m`: `r[n]`'s cross-edge partner is `r[k−1−n]`, so a one-sided trim would leave the innermost arcs ending on a shared edge with nothing to meet. Trimming both ends keeps the ladder self-complementary — symmetric about `SIDE/2` — which is the entire condition for chaining. The band narrows from both sides as density climbs (density 4 keeps 2 of 4, density 16 keeps 12 of 16) and the arcs that survive still S-bend into their neighbours. Verified over densities 1–16: every kept radius has its complement kept, and none exceeds `H`.
